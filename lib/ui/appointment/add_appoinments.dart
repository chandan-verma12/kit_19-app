...
import 'package:kit_19/ui/appointment/pick_mao_location.dart';
...
class AddAppointment extends StatefulWidget {
  static String tag = 'add_appointment';
...
class _AddAppointment extends BaseClass<AddAppointment> implements ApiResponse {
  Lead? lead;
  ...
  late DateTime startDateTime, endDateTime, customNotifySms, customNotifyEmail;
  LatLng? latLng;
  ...
  void initState() {
    ...
    _etWhere = TextEditingController();
    _etCompleteAddress = TextEditingController();
    _etOwner = TextEditingController();
    _etCollaborators = TextEditingController();
    _searchLeadController = TextEditingController();
    WidgetsFlutterBinding.ensureInitialized();
    startDateTime = DateTime.now().add(const Duration(hours: 24));
    endDateTime = DateTime.now().add(const Duration(hours: 25));
    ...
    _etTitle.text =
        "Appointment_${DateTime.now().millisecondsSinceEpoch.toString()}";
    _etFromDate.text = getFormatedDateTime(startDateTime,
        outPutFormat: DateFormats.dd_MMM_yyyy_HH_mm_ss);
    _etToDate.text = getFormatedDateTime(endDateTime,
        outPutFormat: DateFormats.dd_MMM_yyyy_HH_mm_ss);
  ...
  Widget build(BuildContext context) {
    ...
    final etFromDate = TextField(
        ...
        decoration: InputDecoration(
          ...
          hintText: 'From Date and Time',
          enabledBorder: underLineBorder,
          focusedBorder: underLineBorder,
        ));

    final etToDate = TextField(
        ...
        decoration: InputDecoration(
          ...
          hintText: 'To Date and Time',
          enabledBorder: underLineBorder,
          focusedBorder: underLineBorder,
        ));

    final etWhere = TextField(
        controller: _etWhere,
        readOnly: true,
        onTap: () {
          gotoPickLocation();
        ...
        decoration: InputDecoration(
          ...
          hintStyle: TextStyle(
              color:
                  isValidLocation ? AppTheme.colorDarkGrey : AppTheme.colorRed),
          enabledBorder: underLineBorder,
          focusedBorder: underLineBorder,
        ));

    final etCompleteAddress = TextField(
        controller: _etCompleteAddress,
        textCapitalization: TextCapitalization.sentences,
        keyboardType: TextInputType.text,
        decoration: InputDecoration(
    ...
    DatePicker.showDateTimePicker(context, onConfirm: (d) {
      ...
      setState(() {
        ...
        _etFromDate.text = getFormatedDateTime(d,
            outPutFormat: DateFormats.dd_MMM_yyyy_HH_mm_ss);
        _etToDate.text = getFormatedDateTime(d.add(const Duration(hours: 1)),
            outPutFormat: DateFormats.dd_MMM_yyyy_HH_mm_ss);
  ...
  void showToDatePicker() {
    DatePicker.showDateTimePicker(context,
        minTime: startDateTime.add(const Duration(hours: 1)), onConfirm: (d) {
      endDateTime = d;
      setState(() {
        _etToDate.text = getFormatedDateTime(d,
            outPutFormat: DateFormats.dd_MMM_yyyy_HH_mm_ss);
      });
    });
  }
  ...
  void gotoPickLocation() async {
    ...
    if (r != null) {
      ...
      setState(() {
        _etWhere.text = data[Arguments.ADDRESS];
        latLng = data[Arguments.LAT_LNG] as LatLng;
        isValidLocation = true;
      });
    }
  }
    ...
    if (validateForm()) {
      ...
      final Map<String, dynamic> params = {
        ...
        'Details': {
          ...
          "StartDate": _etFromDate.text.trim(),
          "EndDate": _etToDate.text.trim(),
          "IsTask": false,
          "IsAppointment": true,
          "IsCompleted": false,
          "Address": _etWhere.text.trim(),
          "Latitude": latLng?.latitude,
          "Longitude": latLng?.longitude,
          "CompleteAddress": _etCompleteAddress.text.trim(),
          "AssignedTo":
...
*/
  ...
  void setData() {
    ...
    _etWhere.text = taskDetails!.where!;
    _etCompleteAddress.text = taskDetails!.completeAddress!;
    latLng = LatLng(double.parse(taskDetails!.latitude!),
        double.parse(taskDetails!.longitude!));
    if (taskDetails != null) {
      if (taskDetails?.collaborators != null) {
        List<String> colabs = [];
        for (int i = 0; i < taskDetails!.collaborators!.length; i++) {
    ...
    endDateTime = getDateTimeFromString(
        taskDetails!.endDate!, DateFormats.dd_MMM_yyyy_HH_mm_ss);
    customNotifyEmail = startDateTime.subtract(const Duration(minutes: 15));
    customNotifySms = startDateTime.subtract(const Duration(minutes: 15));
    _etFromDate.text = getFormatedDateTime(startDateTime,
        outPutFormat: DateFormats.dd_MMM_yyyy_HH_mm_ss);
